...
| S2d eg/dyn fixed-space shadows | `eg-dyn-shadows` | worktree | agent (relaunch) | **relaunched (hardened)** | 1st attempt edited main directly (mechanism: absolute `/Users/n/Projects/EvoCA/…` paths + `cd` to main; evidence in transcript — first Edit targeted main, 28 main-path ops). Recovered to HEAD; partial work in `S2D_INCOMPLETE.patch`. Relaunched alone with path-jail prompt + orchestrator pre/post clean-check. | 2026-05-16 |
